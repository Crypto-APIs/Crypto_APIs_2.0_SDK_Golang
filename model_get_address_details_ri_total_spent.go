/*
CryptoAPIs

Crypto APIs 2.0 is a complex and innovative infrastructure layer that radically simplifies the development of any Blockchain and Crypto related applications. Organized around REST, Crypto APIs 2.0 can assist both novice Bitcoin/Ethereum enthusiasts and crypto experts with the development of their blockchain applications. Crypto APIs 2.0 provides unified endpoints and data, raw data, automatic tokens and coins forwardings, callback functionalities, and much more.

API version: 2.0.0
Contact: developers@cryptoapis.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package cryptoapis

import (
	"encoding/json"
)

// GetAddressDetailsRITotalSpent struct for GetAddressDetailsRITotalSpent
type GetAddressDetailsRITotalSpent struct {
	// Defines the total amount of all spent by this address coins, based on confirmed transactions.
	Amount string `json:"amount"`
	// Represents the unit of the total spent amount.
	Unit string `json:"unit"`
}

// NewGetAddressDetailsRITotalSpent instantiates a new GetAddressDetailsRITotalSpent object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetAddressDetailsRITotalSpent(amount string, unit string) *GetAddressDetailsRITotalSpent {
	this := GetAddressDetailsRITotalSpent{}
	this.Amount = amount
	this.Unit = unit
	return &this
}

// NewGetAddressDetailsRITotalSpentWithDefaults instantiates a new GetAddressDetailsRITotalSpent object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetAddressDetailsRITotalSpentWithDefaults() *GetAddressDetailsRITotalSpent {
	this := GetAddressDetailsRITotalSpent{}
	return &this
}

// GetAmount returns the Amount field value
func (o *GetAddressDetailsRITotalSpent) GetAmount() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Amount
}

// GetAmountOk returns a tuple with the Amount field value
// and a boolean to check if the value has been set.
func (o *GetAddressDetailsRITotalSpent) GetAmountOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Amount, true
}

// SetAmount sets field value
func (o *GetAddressDetailsRITotalSpent) SetAmount(v string) {
	o.Amount = v
}

// GetUnit returns the Unit field value
func (o *GetAddressDetailsRITotalSpent) GetUnit() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Unit
}

// GetUnitOk returns a tuple with the Unit field value
// and a boolean to check if the value has been set.
func (o *GetAddressDetailsRITotalSpent) GetUnitOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Unit, true
}

// SetUnit sets field value
func (o *GetAddressDetailsRITotalSpent) SetUnit(v string) {
	o.Unit = v
}

func (o GetAddressDetailsRITotalSpent) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["amount"] = o.Amount
	}
	if true {
		toSerialize["unit"] = o.Unit
	}
	return json.Marshal(toSerialize)
}

type NullableGetAddressDetailsRITotalSpent struct {
	value *GetAddressDetailsRITotalSpent
	isSet bool
}

func (v NullableGetAddressDetailsRITotalSpent) Get() *GetAddressDetailsRITotalSpent {
	return v.value
}

func (v *NullableGetAddressDetailsRITotalSpent) Set(val *GetAddressDetailsRITotalSpent) {
	v.value = val
	v.isSet = true
}

func (v NullableGetAddressDetailsRITotalSpent) IsSet() bool {
	return v.isSet
}

func (v *NullableGetAddressDetailsRITotalSpent) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetAddressDetailsRITotalSpent(val *GetAddressDetailsRITotalSpent) *NullableGetAddressDetailsRITotalSpent {
	return &NullableGetAddressDetailsRITotalSpent{value: val, isSet: true}
}

func (v NullableGetAddressDetailsRITotalSpent) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetAddressDetailsRITotalSpent) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


